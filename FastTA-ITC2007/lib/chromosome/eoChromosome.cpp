...
//            if (front_load_periods.Contains(periods.GetById(n.old_period_id))
//                    && !front_load_periods.Contains(periods.GetById(n.new_period_id)))
//                fitness -= model_weightings.Get().front_load[2];
//            //Adjust front load if necessary
//            else if (!front_load_periods.Contains(periods.GetById(n.old_period_id))
//                     && front_load_periods.Contains(periods.GetById(n.new_period_id)))
//                fitness += model_weightings.Get().front_load[2];
//        }
//
...
//    vector<pair<int, int> > roomExams = getRoomExams(_period, _room);
    // Get exam vector
    auto &examVector = getExamVector();
    // Fitness
    int fitness = 0;

//    for (auto const& examCountsPair : roomExams) {
//        // Get exam id
//        int exam_id = examCountsPair.first;
//        // Get exam duration
//        int curr_duration = examVector[exam_id]->getDuration();
//        // See if duration is different from the distinct ones contained in vector sizes
//        if (std::find_if(sizes.begin(), sizes.end(),
...
//        vector<int> sizes;
        ...
        for(auto const &examRoomTuple : periodExams)
        {
            ...
            int exam_id = std::get<0>(examRoomTuple);
            // See the scheduled exam duration
//            if (timetableCont.getRoom(exam_id, _period) == room_id) {
...
//        if (period2_id >= timetableCont.getNumPeriods())
//            continue;
...
//        // If exam ej is scheduled in period pj
//        if (timetableCont.isExamScheduled(ej, period2_id)) {
//            // Get number of conflicts between exams
//            int no_conflicts = conflictMatrix.getVal(_ei, ej);
...
//    //        two_exams_in_a_row += no_conflicts * model_weightings.two_in_a_row;
//            sourceExamProximityConflicts += no_conflicts * model_weightings.two_in_a_row;
//        }
...
//            // If exam ej is scheduled in period periodj_id
//            if (timetableCont.isExamScheduled(ej, periodj_id)) {
//                // Get number of conflicts between exams
//                int no_conflicts = conflictMatrix.getVal(_ei, ej);
...
////                two_exams_in_a_day += no_conflicts * model_weightings.two_in_a_day;
//                sourceExamProximityConflicts += no_conflicts * model_weightings.two_in_a_day;
//            }
//        }
...
//        if ( (distanceToFeasibility = ptrConstraint->operator()(*chrom)) > 0) {
///// COULD BE OPTIMIZED
/////
/////
//            // Determine exams allocated to room rk in period tj
//            vector<pair<int,int> > roomExams = chrom->getRoomExams(_tj, _rk);
////            // Get scheduled rooms vector
...
//    for (int exam_j = 0; exam_j < getNumExams(); ++exam_j) {
//        // If exam exam_j is scheduled in period tj
//        if (timetableCont.isExamScheduled(exam_j, _tj) && timetableCont.getRoom(exam_j, _tj) == _rk) {
//            // Get # student enrolled for exam_j
//            int thisExamNumStudents = examVector[exam_j]->getNumStudents();
//            // Add exam to vector
//            roomExams.push_back(make_pair(exam_j, thisExamNumStudents));
//        }
//    }
