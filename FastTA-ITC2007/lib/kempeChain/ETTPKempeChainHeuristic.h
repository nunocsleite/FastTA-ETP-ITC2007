...
private:
     ...
     */
    enum class Operator { RoomMove, ShiftMove, SwapMove, SlotMove, ShiftMoveIfFeasible };
...
protected:
    ...
    /**
     * @brief shiftMoveIfFeasible
     * @param _sol
     */
    void shiftMoveIfFeasible(EOT &_sol);
...
void ETTPKempeChainHeuristic<EOT>::doSolutionMove(EOT &_sol) {
    ...
    if (currentOperator == Operator::ShiftMove || currentOperator == Operator::ShiftMoveIfFeasible) {
...
//            if (++numTries > 3) {
    ...
    ///////////////////////////////////////////////////////////////////////////////////
    else if (currentOperator == Operator::ShiftMoveIfFeasible) {
        // Get source exam from conflict vector
        int examSource = std::get<0>(_conflictingExamsTsource.back());
        // Select random room with sufficient capacity for hosting _examSource
        int roomDest;
        ...
        getSourceExamHardConflictsDestPeriod(examSource, examGraph, _tDest, roomDest, _conflictingExamsTdest);
        // If there are no conflicts, schedule exam
        if (_conflictingExamsTdest.empty()) {
            // The neigbour solution is feasible
            //
            // Unschedule exam from source time slot
            //
            timetableCont.unscheduleExam(examSource, _tSource);
            ...
            // Remove sourceExam from source conflicting exams
            _conflictingExamsTsource.pop_back();
            // The neigbour solution is infeasible
            setNeighbourFeasibility(false);
            return;
        }
...
//    //
//    // A set containing the rooms with sufficient capacity is formed, and a random room is selected.
//    //
...
//    for (int room = 0; room < _sol.getNumRooms(); ++room) {
////        if (_sol.verifyRoomCapacityConstraint(_ei, room)) {
//        if (/*_sol.verifyRoomCapacityConstraint(_ei, room) &&*/
//            _sol.verifyRoomOccupancyConstraint(_ei, _tj, room) &&
//            _sol.verifyRoomRelatedConstraints(_ei, _tj, room) /*&&
//            _sol.verifyPeriodRelatedConstraints(_ei, _tj)*/) {
 ...
 */
template <typename EOT>
void ETTPKempeChainHeuristic<EOT>::shiftMoveIfFeasible(EOT &_sol) {
    //
    // 2. Shift move - Here a random exam is moved into different
    // (randomly chosen) timeslot and room.
    //
    // Set operator type
    currentOperator = Operator::ShiftMoveIfFeasible;
