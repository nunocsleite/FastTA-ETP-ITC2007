...
private:
     ...
     * @return
     */
    static int computePriority(EOT &_chrom, int _ej, vector<vector<int> > &_examsAvailablePeriodsList, vector<bool> &_scheduledExams);
...
private:
     ...
     */
    static void unscheduleConflictingExams(EOT &_chrom, vector<vector<int> > &_examsAvailablePeriodsList,
...
/////////////////////////////////////////////////////
/// NOT WORKING CODE WITH DEBUG MESSAGES
///
///**
// * @brief saturationDegree
// * @param _timetableProblemData
...
// */
//template <typename EOT>
//void GCHeuristics<EOT>::saturationDegree(const TimetableProblemData &_timetableProblemData, EOT& _chrom) {
//    //===
...
//    int rk = -1;
//#ifdef GRAPH_COLOURING_HEURISTIC_DEBUG_CONSTRAINTS
//    // Get exam vector
//    cout << "Room hard constraints:" << endl;
//    // Get exam vector
//    auto const &examVector = chrom->getExamVector();
//    for (auto const &exam_i : examVector) {
//        int ei = (*exam_i.get()).getId();
////        cout << "ei = " << ei << endl;
//        // Get exam 'ei' Room-Related Hard constraints
//        auto const& roomRelatedHardConstraintsVector = examVector[ei]->getRoomRelatedHardConstraints();
//        // Print constraints
//        for (auto const& ptrConstraint : roomRelatedHardConstraintsVector) {
//            boost::shared_ptr<UnaryConstraint> ptrUnaConstraint =
//                    boost::dynamic_pointer_cast<UnaryConstraint, Constraint>(ptrConstraint);

//            cout << "ei = " << ptrUnaConstraint->getE()
//                 << ", isScheduled ? " << examVector[ptrUnaConstraint->getE()]->isScheduled() << endl;
//        }
//    }
//#endif
...
//#ifdef GRAPH_COLOURING_HEURISTIC_DEBUG_10
//        cout << "ei = " << ei << endl;
//        Exam &scheduledExam = *chrom->getExamVector()[ei].get();
//        cout << "ei # students = " << scheduledExam.getNumStudents() << endl;
//        cout << "ei duration = " << scheduledExam.getDuration() << endl;
//        cout << "numAvailablePeriods = " << numAvailablePeriods << endl;
//        // Room info
//        cout << "Room info:" << endl;
...
//#ifdef GRAPH_COLOURING_HEURISTIC_DEBUG_2
//            cout << "numAvailablePeriods = " << numAvailablePeriods << endl;
//            cout << "examsAvailablePeriodsList[" << ei << "].size() = " << examsAvailablePeriodsList[ei].size() << endl;
//            if (numAvailablePeriods > 0 && examsAvailablePeriodsList[ei].size() == 0) {
//                cout << "ERROR!" << endl;
//                cin.get();
//            }
//#endif
//            // Period tj and room rk are out parameters and are updated
//            examWasScheduled = tryScheduleExam(ei, tj, rk, examsAvailablePeriodsList);

//#ifdef GRAPH_COLOURING_HEURISTIC_DEBUG_10
//            cout << "Exam scheduled ? " << (examWasScheduled ? "true" : "false") << endl;
//            if (examWasScheduled) {
//                cout << "Exam ei = " << ei << " info:" << endl;
//                cout << "ei # students = " << scheduledExam.getNumStudents() << endl;
//                cout << "tj = " << tj << endl;
//                cout << "rk = " << rk << endl;
//                // Get room vector
...
//                int numOccupiedSeats = roomVector[rk]->getNumOccupiedSeats(tj);
//                cout << "rk occupied seats = " << numOccupiedSeats << endl;
//            }
//            cin.get();
//#endif
//        }
...
//#ifdef GRAPH_COLOURING_HEURISTIC_DEBUG_10
//            cout << "Exam was not scheduled -> going to step 2.4" << endl;
////            cout << "# already scheduled exams = " << numScheduledExams << endl;
//            cin.get();
//#endif
//            //   2.4 - SelectValue - If the available period list for exam 'ei' is empty, or the
//            //         selected period and room is not feasible (at least one of the constraints 2-5 is violated),
...
//            i2tuple value = selectValue(ei, eiHardConflicts, domainValues);
//            // Update tj
//            tj = std::get<0>(value);
//            // Get room rk
//            rk = std::get<1>(value);
//            // Unschedule all conflicting exams, and put them in the priority queue again
//            unscheduleConflictingExams(pq, eiHardConflicts, scheduledExams, numScheduledExams);
//            // Schedule 'ei' into period and room
//            scheduleExam(ei, value);
...
////            _chrom.validate();
//////            cout << "Finished step 2.4" << endl;
//////            cout << "eiHardConflicts.size() = " << eiHardConflicts.size() << endl;
//////            cin.get();
////#endif
...
//            // # AvailablePeriods
//            int numAvailablePeriods = std::get<1>(eiVariable);
//            cout << "-> going to step 2.5 - Update period availability" << endl;
////            cin.get();
//#endif

////        if (examWasScheduled) {
//            updatePeriodAvailability(ei, tj, scheduledExams, examsAvailablePeriodsList, pq);
...
////#ifdef GRAPH_COLOURING_HEURISTIC_DEBUG
////        else
////            cout << "exam " << ei << " was not scheduled" << endl;
////#endif
...
//    // SD construction algorithm
//    //
//    // 1. Create a priority queue with the exams sorted randomly. Only exams that have the 'After' hard constraint
//    //    have less available periods and are scheduled first. The others exams have, initially, all available periods.
//    //
//    // 2. while not Complete(sigma) and not Timeout do
//    //    2.1     v = SelectVariable(sigma)
...
//            rk = std::get<1>(value);
            ...
            // Unschedule all conflicting exams, and put them in the priority queue again
            unscheduleConflictingExams(_chrom, examsAvailablePeriodsList, pq, eiHardConflicts, scheduledExams, numScheduledExams);
        }
//        else {
            //   2.5. Update the period availability for all unscheduled exams according to the 'No-Conflicts' hard constraint.
            //
...
template <typename EOT>
int GCHeuristics<EOT>::computePriority(EOT &_chrom, int _ej, vector<vector<int> > &_examsAvailablePeriodsList,
    ...
    // Flag used to check if period pi is feasible
    bool feasiblePeriodPi;

    // Clear available periods list for exam ej
    _examsAvailablePeriodsList[_ej].clear();
    //-
    // Verify the No-Conflicts hard constraint within period pi
    // No-Conflicts: Conflicting exams cannot be assigned to the same period. (As usual,
    //   two exams are said to conflict whenever they have some student taking them both.)
    //-
    // For each period do
    for (int pi = 0; pi < timetableCont.getNumPeriods(); ++pi) {
        // Period pi is initially feasible
        feasiblePeriodPi = true;
        // For each exam of period pi do
        for (int exam_i = 0; exam_i < _chrom.getNumExams(); ++exam_i) {
            // If exam exam_j is scheduled, is different from ei and is scheduled in period pi
            if (_scheduledExams[exam_i] && exam_i != _ej && timetableCont.isExamScheduled(exam_i, pi)) {
                // Get conflicts from the conflicting matrix
                int n = conflictMatrix.getVal(exam_i, _ej);
                if (n > 0) {
                    feasiblePeriodPi = false;
                    break;
                }
            }
        }
        if (feasiblePeriodPi) {
            // If period pi is feasible, then add it to the ej's feasible periods list
            _examsAvailablePeriodsList[_ej].push_back(pi);
        }
    }
    //
    // Return ej's priority as the number of available periods in the timetable
    return _examsAvailablePeriodsList[_ej].size();
}
...
///
        ...
        while (roomExams.size() > 0 && numOccupiedSeats + thisExamNumStudents > roomSeatCapacity) {
            ...
            /////////////////////////////////////////////////////////////
//            // Compute number of # occupied seats for this room
//            int computedNumOccupiedSeats = 0;
//            // Exam # students
//            int examStudents;
//            // For each exam in room do
//            for (auto const& examNumStudentsPair : roomExams)
//            {
//                // Get exam_id
//                int exam_id = examNumStudentsPair.first;
//                // Get num students for this exam
//                int examStudents = examNumStudentsPair.second;
//                // Get real num students
//                int examStudents1 = examVector[exam_id]->getNumStudents();

//                cout << "examStudents = " << examStudents << ", examStudents1 = " << examStudents1 << endl;

//                if (examStudents != examStudents1) {
//                    cout << "examStudents != examStudents1" << endl;
//                    cin.get();
//                }
//                // Sum num occupied seats
...
//            }
//            cout << "computedNumOccupiedSeats = " << computedNumOccupiedSeats << ", numOccupiedSeats = " << numOccupiedSeats << endl;

//            if (computedNumOccupiedSeats != numOccupiedSeats) {
//                cout << "computedNumOccupiedSeats != numOccupiedSeats" << endl;
//                cin.get();
//            }
            /////////////////////////////////////////////////////////////
 ...
 */
template <typename EOT>
void GCHeuristics<EOT>::unscheduleConflictingExams(EOT &_chrom, vector<vector<int> > &_examsAvailablePeriodsList,
                                                   VertexPriorityQueue &_pq,
                                                   std::vector<VariableValueTuple> const &_eiHardConflicts,
                                                   vector<bool> &_scheduledExams,
    ...
    for (VariableValueTuple b : _eiHardConflicts) {
        ...
        int ej = std::get<0>(b);
        // Compute ej's priority (number of available periods in the timetable)
        int priority = computePriority(_chrom, ej, _examsAvailablePeriodsList, _scheduledExams);
...
void GCHeuristics<EOT>::updatePeriodAvailability(EOT &_chrom, int _ei, int _tj, std::vector<bool> _scheduledExams,
    ...
    // 2.5. Update the period availability for all unscheduled exams according to the 'No-Conflicts' hard constraint.
    //
    // 2.5.1. Mark period 'tj' of all the exams connected to 'ei' as unavailable, because they have common registered students.
